...
for R1 in "$TRIMMED_DIR"/paired/*_R1_paired.fastq.gz; do  
    ...
    KRAKEN_CMD="kraken2 --d \"$DATABASE\" --threads 8 --report \"$REPORTS_DIR/${base}.k2report\" \
               --memory-mapping --report-minimizer-data --paired --minimum-hit-groups 2 \
...
if cp $ROOT_DIR/scripts/logs/*_"$SLURM_JOB_ID".* "$LOG_DIR"; then
    ...
    python "$ROOT_DIR/scripts/helper_scripts/evaluation_metrics.py" "$RUN_DIR" 2>&1 || { echo "❌ Evaluation metrics generation failed!"; exit 1; }
